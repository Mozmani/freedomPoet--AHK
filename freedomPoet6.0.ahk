...
;					temp := a_index
				...
				curManaP := ((100 / maxMana) * curMana)
					...
					loop % groupSize {
						...
						temp := a_index 
							...
							if (90 < vitaP%a_index%)
								waitToHeal%a_index% = 0
							if (90 > vitaP%a_index%) and (waitToHeal%a_index% = 1) {
								waitToHeal%a_index% = 1
								healMeTimer%a_index% := a_tickcount
								}
							if ((a_tickcount-healMeTimer%a_index%) > 10000) and (waitToHeal%a_index% = 0) and (spellCount%poetTemp% < 3) and (curMana>(healCost+30)) {
				...
				invokeOverlay%a_index% := 20 - invokeOverlay%a_index%
				setformat, float, 0
...
tooltip, freedomPoet: %castSpellsState% | castScourge: %castScourgeState%, 0, 0 
	} ;end infinite loop
return
